...
3b. For v2026.6 and later, create a directory in configs named "ANSN" before creating the `MERLYN435` directory inside "ANSN"
...
2. **Via SWD:** If the bootloader is corrupted, connect an ST Link programmer to the `B`, `D` `C` test points (`R` is optional) and flash the compiled binary with STM32CubeProgrammer.

## ExpressLRS Flashing
The internal ESP8285 ExpressLRS receiver is wired to the AT32 via UART2. It uses the **BETAFPV 2.4GHz Lite RX** firmware target. The firmware binaries can be optained from the ELRS [site](https://expresslrs.github.io/web-flasher/) or from this repository in [/firmware/binaries](https://github.com/YeetTheAnson/Merlyn435-Flight-Stack/blob/main/firmware/binaries/(ExpressLRS)MERLYN435_v4.10_FCC.bin). There's two way to flash the receiver:
...
If the ESP8285 is bricked or passthrough fails, you can flash it directly using an FTDI adapter or another ESP8285/ESP8286/ESP32 or even an arduino. Bridge `R` to `GND` as well as the BOOT jumper and connect the FTDI TX to RX and FDTI RX to TX
...
Note: The prices in USD are converted as of 22 August 2026 and may fluctuate.
...
| Flight Controller | APH0420T4R7M | 3.7uH inductor | [link](https://www.lcsc.com/product-detail/C5349668.html) | LCSC | 5 | 0.45 |
...
| Flight Controller | 1206B475K500NT | 4.7uH 1206 capacitor | [link](https://www.lcsc.com/product-detail/C29823.html) | LCSC | 10 | 2.72 |
...
| ESC | HoYLR2512E-3W-0.5mR-1% | 0.5m 2512 shunt resistor | [link](https://www.lcsc.com/product-detail/C5375456.html) | LCSC | 10 | 0.60 |
...
**Fligalht Controller Total:** \$96.41
**ESC Total:** \$42.02
